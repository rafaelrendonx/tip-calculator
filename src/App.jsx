...
*/
  ...
  const tipAmountPerPerson = Number(((bill * (tip / 100)) / people).toFixed(2)) || 0.00
  const totalAmountPerPerson = Number((Number(tipAmountPerPerson) + (bill / people)).toFixed(2)) || 0.00
  const customTipAmountPerPerson = Number(((bill * (customTip / 100)) / people).toFixed(2)) || 0.00
  const customTotalAmountPerPerson = Number(((Number(customTipAmountPerPerson)) + (bill / people)).toFixed(2)) || 0.00
      ...
      <main className=" bg-white p-8 rounded-t-3xl">
        ...
        <div className="flex flex-col pb-[30px]">
          ...
          <div className="relative rounded-md border">
            <input type="number" value={bill} onChange={handleBill} id="bill" min="0"
              className="rounded-md py-[6.75px] bg-very-light-grayish-cyan text-very-dark-cyan text-2xl text-right pr-2 focus:outline-none focus:ring focus:ring-strong-cyan border" />
        ...
        <div className="pb-[30px]">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <input type="number" value={customTip} onChange={handleCustomTip} placeholder="Custom" onClick={() => handleTip(customTip, 5)} id="custom" min="0"
              className="text-very-dark-cyan text-right text-2xl bg-very-light-grayish-cyan placeholder:text-center placeholder:text-very-dark-cyan p-[7px] rounded-md focus:outline-none focus:ring focus:ring-strong-cyan border pl-5" />
          </div>
        </div>
          ...
          <div className="relative rounded-md border">
            {people === 0 ?
              <input type="number" value={people} onChange={handlePeople} id="people" min="0"
                className="rounded-md py-[6.75px] bg-very-light-grayish-cyan text-very-dark-cyan text-2xl text-right pr-2 focus:outline-none focus:ring focus:ring-red-400 border" />
              :
              <input type="number" value={people} onChange={handlePeople} id="people" min="0"
                className="rounded-md py-[6.75px] bg-very-light-grayish-cyan text-very-dark-cyan text-2xl text-right pr-2 focus:outline-none focus:ring focus:ring-strong-cyan border" />}
